...
import { RefreshTokenRepo, ResetTokenRow, ResetTokenRepo } from '../repositories/token.repo';
import { hashPassword, verifyPassword, randomToken, sha256 } from '../utils/crypto';
...
export const AuthService = {
  async register(input: { name: string; email: string; password: string }) {
    ...
    const user = await UserRepo.create({ name: input.name, email: input.email, password_hash });
    const accessToken = signAccessToken({ sub: user.id, role: user.role });
    const refresh = await this.issueRefreshToken(user.id);
    return { user: sanitizeUser(user), accessToken, refreshToken: refresh.token }; // return JSON tokens (or set cookies in controller)
  },
  ...
  async login(input: { email: string; password: string }) {
    ...
    const accessToken = signAccessToken({ sub: user.id, role: user.role });
    const refresh = await this.issueRefreshToken(user.id);
    return { user: sanitizeUser(user), accessToken, refreshToken: refresh.token };
  },

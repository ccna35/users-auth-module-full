...
export const UserRepo = {
  ...
  async create(input: { name: string; email: string; password_hash: string; role?: 'USER' | 'ADMIN' }): Promise<UserRow> {
  ...
  async list(opts: { page: number; pageSize: number; search?: string; status?: string; role?: string }) {
    ...
    const [cntRows] = await pool.query<{ c: number }[]>(
      `SELECT COUNT(*) as c FROM users ${whereSql}`, params
    );

    return { items: rows, total: cntRows[0].c };
  },

  async update(id: string, fields: Partial<Pick<UserRow, 'name'|'email'|'role'|'status'>>): Promise<UserRow> {
    const sets: string[] = [];
    const params: any[] = [];
  ...
  async softDelete(id: string) {
    await pool.execute('UPDATE users SET status = ? WHERE id = ?', ['DELETED', id]);
  }
};
